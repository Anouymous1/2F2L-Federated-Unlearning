...
class Trainer:
    ...
    def _initialize_model(self):
        if self.args.dataset == "mnist":
            ...
            raise NotImplementedError(
                f"Model for dataset {self.args.dataset} not implemented.")
    ...
    def train(self, train_loader, loss_fun, optimizer):
        ...
        for imgs, labels in train_loader:
            imgs, labels = imgs.to(self.device), labels.to(
                self.device).view(-1)
            num_data += labels.size(0)
            outputs, _ = self.model(imgs)
            loss = loss_fun(outputs,
                            labels) + (0.0 / 2) * L2_Regularization(self.model)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
class Pretrainer:
    ...
    def run(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            train_loss, train_acc = self.model.train(train_loader_server,
                                                     loss_func, self.optimizer)
            ...
            if epoch % self.args.save_freq == 0:
                self.model.save_model(
                    self.args.save_path,
                    self.filename + "_warmup_epoch_" + str(epoch))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--server_rate",
                        type=float,
                        default=0.001,
    ...
    parser.add_argument("--dataset",
                        ...
                        default="mnist",
                        help="which dataset")
    parser.add_argument("--imgnet_path",
                        type=str,
                        default="",
                        help="path to imagenet pretrained model")
