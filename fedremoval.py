...
class FedRemovalTrainer:
    ...
    def __init__(self, args, logger, file_manager):
        ...
        self.pretrain_filename = os.path.join(
            "./checkpoint/warmup_ce", args.dataset,
    ...
    def init_model(self):
        if args.dataset == "mnist":
            ...
            self.server_model = CLIPLP(num_classes=20,
                                       backbone="ViT-B/32").to(self.device)
            self.pretrained_model = CLIPLP(num_classes=20,
                                           backbone="ViT-B/32").to(self.device)
            self.model_forget = CLIPLP(num_classes=20,
                                       backbone="ViT-B/32").to(self.device)
        elif args.dataset == "domainnet_clipvit":
            self.server_model = CLIPLP(num_classes=10,
                                       backbone="ViT-B/32").to(self.device)
            self.pretrained_model = CLIPLP(num_classes=10,
                                           backbone="ViT-B/32").to(self.device)
            self.model_forget = CLIPLP(num_classes=10,
                                       backbone="ViT-B/32").to(self.device)
        ...
        checkpoint = torch.load(
            os.path.join("checkpoint/backdoor_train", args.dataset,
                         self.train_filename))
    ...
    def collect_grad(self):
        ...
        for i in range(len(self.train_loader_users)):
            if i not in self.args.remove_idx:
                ...
                for idx, (img,
                          labels) in enumerate(self.train_loader_users[i]):
                    img = img.to(self.device)
                    labels = labels.to(self.device)
                    total_samples += labels.size(0)
                    labels_onehot = F.one_hot(
                        labels, self.server_model.num_classes).float()
                    _, jvp = self.server_model(img)
                    output, jvp2 = self.pretrained_model(img)
                    loss = self.loss_mse_func_sum(output + (jvp - jvp2),
                                                  labels_onehot)
                    loss /= 2
                    loss.backward()
    ...
    def eval(self, model):
        with torch.no_grad():
            ...
            self.logger.info("Test Loss: {:.4f} | Test Acc: {:.4f}%".format(
                loss_test, acc_test * 100))

            for idx_forgotten in args.remove_idx:
                loss_test, acc_test = self.test_epoch(
                    model, self.train_loader_users[idx_forgotten])
                self.logger.info(
                    " {:<4s}| Server Backdoor Test Loss: {:.4f} | Server Backdoor Success Rate: {:.4f}%"
                    .format(str(idx_forgotten), loss_test, acc_test * 100))
    ...
    def train(self):
        ...
        """
        self.logger.info(
            "============ Test the Server Model after FL ============")
        ...
        for a_iter in range(args.num_epochs):
            self.logger.info(
                "============ Removal train epoch {} ============".format(
                    a_iter))
            loss_removal = self.train_epoch()
            self.scheduler.step()
            self.logger.info("Removal Loss: {:16f}".format(loss_removal))

            if a_iter % 5 == 0 or (loss_removal < min_loss and a_iter >= (
                    args.num_epochs * 0.8)) or a_iter == args.num_epochs - 1:
                with torch.no_grad():
                    ...
                    for name, param in model_forget_test.named_parameters():
                        if 'jvp' not in name:
                            if param.requires_grad:
                                param.data = self.server_model.state_dict(
    ...
    def train_epoch(self):
        ...
        for i, (img, _) in enumerate(self.train_loader_server):
            ...
            loss /= 2
            l2_penaly = sum([
                torch.norm(p)**2 for p in self.model_forget.parameters()
                if p.requires_grad
            ])
...
class FedRemovalLauncher:
    ...
    def __init__(self, args):
        ...
        self.logger = self.setup_logger(log_path,
                                        self.file_manager.remove_filename())
        self.log()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log",
                        action="store_true",
                        help="whether to make a log")
    parser.add_argument("--wandb",
                        action="store_true",
                        help="whether to make a wandb record")
    parser.add_argument(
        "--server_rate",
        type=float,
        default=0.001,
    ...
    parser.add_argument("--removal_lr",
                        type=float,
                        default=1e-2,
                        help="removal learning rate")
    parser.add_argument("--lr",
                        type=float,
                        default=1e-2,
                        help="trainig learning rate")
    ...
    parser.add_argument("--batch_size",
                        ...
                        default=32,
                        help="batch size")
    parser.add_argument("--warmup_batch_size",
                        type=int,
                        default=32,
                        help="warmup batch size")
    parser.add_argument("--remove_idx",
                        nargs="+",
                        type=int,
                        help="client(s) to be removed")
    parser.add_argument(
        "--eval_from",
        type=int,
        default=5,
    ...
    parser.add_argument("--mu",
                        type=float,
                        default=0.0,
                        help="The hyper parameter for l2 penalty")
    parser.add_argument(
        "--sigma",
        type=float,
        default=0.01,
