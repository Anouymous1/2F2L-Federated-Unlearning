...
class Trainer:
    ...
    def init_model(self):
        if args.dataset == "mnist":
            ...
            self.server_model = CLIPLP(num_classes=20, backbone="ViT-B/32").to(
                self.device).float()
            self.pretrained_model = CLIPLP(
                num_classes=20, backbone="ViT-B/32").to(self.device).float()
        else:
        ...
        self.models = [
            copy.deepcopy(self.server_model).to(self.device)
            for _ in range(self.args.num_users)
        ]
        self.train_loaders = {}
    ...
    def train(self):
        self.logger.info(
            "============ Start Federated Linear Training ============")

        for a_iter in range(self.args.train_epochs):
            self.logger.info(
                "============ Train epoch {} ============".format(a_iter))
        ...
        with torch.no_grad():
            ...
            for idx_forgotten in args.remove_idx:
                loss_test, acc_test = self.test_epoch(
                    self.train_loader_users[idx_forgotten])
                self.logger.info(
    ...
    def aggregation(self, w_locals):
        ...
        if self.args.mode.lower() != "finetune":
            if not self.args.weighted_aggregate:
                client_weights = [
                    1 / args.num_users for i in range(args.num_users)
                ]
            else:
                coeff = 1
                dummy_weights = []
                ...
                client_weights = [
                    dummy_weights[i] / sum(dummy_weights)
                    for i in range(args.num_users)
                ]
    ...
    def train_epoch(
        ...
        for _, (imgs, labels) in enumerate(train_loader):
            ...
            labels_onehot = F.one_hot(labels,
                                      model.num_classes).to(torch.float)
    ...
    def test_epoch(self, test_loader):
        ...
        for _, (imgs, labels) in enumerate(test_loader):
            labels = labels.to(self.device)
            labels_onehot = F.one_hot(labels,
                                      self.server_model.num_classes).float()
            imgs = imgs.to(self.device)
            num_data += labels.size(0)
            if self.args.mode == "mixlinear":
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weighted_aggregate",
                        action="store_true",
                        help="whether to make weighted aggregate")
    parser.add_argument(
        "--server_rate",
        type=float,
        default=0.1,
